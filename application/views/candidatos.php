...
if ($menu2 == 'ListaCandidatos') {
    ...
    if (isset($candidatos)) {
        foreach ($candidatos as $linha) {
            ...
            echo anchor('Candidatos/view/'.$linha -> pr_candidato, '<i class="fa fa-lg mr-0 fa-search">Dados</i>', " class=\"btn btn-sm btn-square btn-warning\" title=\"Dados\"");
            if ($linha -> bl_removido == '0' && $linha -> removido == '0') {
                if ($linha -> pr_usuario != $this -> session -> uid) {
                    if (strlen($linha -> pr_usuario) > 0) {
                        echo anchor('Candidatos/changeName/'.$linha -> pr_candidato, '<i class="fa fa-lg mr-0 fa-edit">Alterar nome</i>', " class=\"btn btn-sm btn-square btn-danger\" title=\"Alterar nome\"");
                        echo anchor('Candidatos/changeEmail/'.$linha -> pr_candidato, '<i class="fa fa-lg mr-0 fa-edit">Alterar e-mail</i>', " class=\"btn btn-sm btn-square btn-danger\" title=\"Alterar e-mail\"");
                        echo "
                                                                                                    <a href=\"javascript:/\" class=\"btn btn-sm btn-square btn-warning\" title=\"Nova senha\" onclick=\"confirm_senha(".$linha -> pr_candidato.");\"><i class=\"fa fa-lg mr-0 fa-envelope\">Nova senha</i></a>
                                                                                                    <a href=\"javascript:/\" class=\"btn btn-sm btn-square btn-danger\" title=\"Desativar usuário\" onclick=\"confirm_delete(".$linha -> pr_candidato.");\"><i class=\"fa fa-lg mr-0 fa-times-circle\">Desativar usuário</i></a>";
                    }
                }
            } elseif (strlen($linha -> pr_usuario) > 0 && $linha -> pr_usuario != $this -> session -> uid) {
                echo "
                                                                                                    <a href=\"javascript:/\" class=\"btn btn-sm btn-success btn-square\" title=\"Reativar usuário\" onclick=\"confirm_reactivate(".$linha -> pr_candidato.");\"><i class=\"fa fa-lg mr-0 fa-plus-circle\">Reativar usuário</i></a>";
            }
            echo "
                                                                                            </td>
                                                                                    </tr>";
    ...
    if (strlen($sucesso) == 0) {
        $attributes = array('class' => 'login-form',
                        'id' => 'form_candidatos');
        echo form_open($url, $attributes, array('codigo' => $codigo));
        echo form_fieldset('Perfil do candidato');
        echo "
                                <div class=\"row\">
                                        ...
                                        <div class=\"col-md-12\">
                                                <div class=\"form-group\">";
        $attributes = array('class' => 'control-label');
        echo form_label('Perfil', 'perfil', $attributes);
